...
-- ============================================================
-- Contenu :
--   1. Trigger helper update_updated_at()
--   2. SECURITY DEFINER helpers (is_org_admin / is_org_member)
...
-- Nécessaires pour éviter la récursion infinie dans les policies RLS
-- de org_members qui référencent org_members.
...
-- Création : tout utilisateur authentifié (onboarding géré en API)
CREATE POLICY "organizations_authenticated_insert"
  ON organizations FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

-- Mise à jour : admins de l'organisation uniquement
CREATE POLICY "organizations_admin_update"
  ON organizations FOR UPDATE
  USING (is_org_admin(id));

-- Suppression : super_admin uniquement (via service role en pratique)
...
-- Lecture : membres de la même org
CREATE POLICY "org_members_member_read"
  ON org_members FOR SELECT
  USING (is_org_member(org_id));

-- Insertion : admins de l'org uniquement
CREATE POLICY "org_members_admin_insert"
  ON org_members FOR INSERT
  WITH CHECK (is_org_admin(org_id));

-- Mise à jour : admins de l'org uniquement
CREATE POLICY "org_members_admin_update"
  ON org_members FOR UPDATE
  USING (is_org_admin(org_id));

-- Suppression : admins de l'org + auto-désinscription
